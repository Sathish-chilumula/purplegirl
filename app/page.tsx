...
export default async function Home() {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* 1. HERO SECTION — High Impact & Balanced */}
      <section className="relative pt-24 pb-32 px-6">
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 bg-purple-50 text-purple-700 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest mb-8 animate-slide-up shadow-sm">
            <Zap size={14} className="fill-purple-700" /> 100% Anonymous Insider Advice
          </div>
          
          <h1 className="font-syne text-5xl md:text-7xl lg:text-8xl font-extrabold text-slate-900 tracking-tight mb-8 leading-[1.1] animate-slide-up stagger-1">
            The Secrets Your <br />
            <span className="text-gradient">Mother Never Told You.</span>
          </h1>
          
          <p className="max-w-2xl text-lg md:text-xl text-slate-500 mb-12 leading-relaxed animate-slide-up stagger-2">
          ...
          <div className="flex flex-col sm:flex-row gap-6 animate-slide-up stagger-3">
            <Link href="/ask" className="btn-premium px-12 py-5 text-lg flex items-center gap-2 group shadow-purple-200">
              Ask A Taboo Question <ArrowRight className="group-hover:translate-x-2 transition-transform" />
            </Link>
            <Link href="#categories" className="px-12 py-5 text-lg font-bold text-slate-900 hover:text-purple-600 transition-all border border-slate-100 rounded-full hover:bg-slate-50">
              Browse The Vault
            </Link>
          </div>
        </div>
      ...
      {/* 4. TRUST STRIP */}
      <section className="bg-slate-900 py-24 px-6 md:px-12 text-center text-white relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="absolute inset-0 bg-gradient-to-tr from-purple-900/40 to-transparent z-0 pointer-events-none" />
          <div className="relative z-10">
            ...
            <h2 className="text-3xl md:text-5xl font-syne font-bold mb-6">Encrypted. Anonymous. <span className="text-purple-400">Safe.</span></h2>
